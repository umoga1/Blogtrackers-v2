...
public class Blogger {
	...
	public ArrayList _getBloggerPostFrequency(String blogids) throws Exception {
		...
		try {
			result = db.query("select distinct blogger_name, sum(blogpost_count) as totalpost from blogger where blogsite_id in "+blogids+" group by blogger_name order by totalpost desc limit 500");		
